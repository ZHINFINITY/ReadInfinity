...
describe('ingestFile', () => {
  ...
  test('marks native absolute paths in-place without external library folders', async () => {
    const { appService, settings, isLoggedIn, importBook } = makeDeps();
    await ingestFile(
  ...
  test('marks in-place when the file matches none of the registered folders', async () => {
    const { appService, settings, isLoggedIn, importBook } = makeDeps({
      externalLibraryFolders: ['/Users/me/Duokan', '/Users/me/Calibre Library'],
    });
    await ingestFile(
  ...
  test('opens native absolute paths in-place on Linux', async () => {
    const { appService, settings, isLoggedIn, importBook } = makeDeps({
  ...
  test('opens native absolute paths in-place on Android', async () => {
    const { appService, settings, isLoggedIn, importBook } = makeDeps({
  ...
  test('managed app paths also remain in-place in offline mode', async () => {
    // The managed app directory is still a local device path in the offline
    // build, so it is intentionally not copied again.
    const { appService, settings, isLoggedIn, importBook } = makeDeps({
      externalLibraryFolders: ['/Users/me/AppData'],
      osPlatform: 'macos',
    });
  ...
  test('opens content-provider URIs in-place', async () => {
    const { appService, settings, isLoggedIn, importBook } = makeDeps({
      osPlatform: 'android',
    });
    await ingestFile(
      { file: 'content://com.android.providers.downloads.documents/document/123', books: [] },
      { appService, settings, isLoggedIn },
    );
    expect(importBook.mock.calls[0]?.[2]).toMatchObject({ inPlace: true });
  });
  ...
  test('native absolute paths use the byFilePath fast path without a registered root', async () => {
    const { appService, settings, isLoggedIn, importBook } = makeDeps();
  ...
  test('never queues an upload for a native in-place book', async () => {
    const { appService, settings, isLoggedIn, importBook } = makeDeps({
      isLoggedIn: true,
